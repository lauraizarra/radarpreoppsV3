...
function getExecutiveState(preopp: Pick<PreOpp, "etapa" | "estado">): ExecutiveState {
  if (["Propuestas", "Activas", "Convertidas", "Convertida Congelada", "Descartadas"].includes(preopp.estado)) return preopp.estado as ExecutiveState;
  ...
  if (["Convertida a Cloud Sales", "Convertida a Prospect", "Prospect", "Qualified", "Technical Validation", "Business Validation", "Committed", "Commited", "Launched"].includes(preopp.etapa)) return "Convertidas";
...
function getAlert(preopp: PreOpp) {
  ...
  if (["Convertida a Prospect", "Convertida a Cloud Sales", "Prospect", "Qualified", "Technical Validation", "Business Validation", "Committed", "Commited", "Launched"].includes(preopp.etapa)) return "Convertida";
  if (preopp.etapa === "Frozen") return "Convertida congelada";
  return "OK";
}
...
function getStageTone(stage: string): Tone {
  ...
  if (["Convertida a Prospect", "Convertida a Cloud Sales", "Prospect", "Qualified", "Technical Validation", "Business Validation", "Committed", "Commited", "Launched"].includes(stage)) return "green";
...
function StatePill({ state }: { state: string }) {
  return <Pill tone={getStateTone(state)}>{state}</Pill>;
}

function KpiCard({ emoji, label, value, hint, tone = "blue" }: { emoji: string; label: string; value: string | number; hint: string; tone?: Tone }) {
  return (
    <article className={`kpi kpi-${tone}`}>
      ...
      <div className="kpi-emoji" aria-hidden="true">{emoji}</div>
    </article>
  );
}
    ...
    <article className="kpi kpi-blue pipeline-kpi">
      <div className="kpi-copy pipeline-copy">
        ...
        <div className="pipeline-breakdown">
          <div><b>{compactCurrency(esperado)}</b><small>Esperado</small></div>
          <div><b>{percent(avance)}</b><small>Avance</small></div>
        </div>
      </div>
      <div className="kpi-emoji" aria-hidden="true">💰</div>
    </article>
  );
}
...
const titles: Record<View, { title: string; description: string }> = {
  overview: { title: "Overview", description: "Estado de cuentas por producto con una sola fila por cuenta." },
  preopps: { title: "Pre-oportunidades", description: "Progreso semanal de pre-oportunidades abiertas o descartadas." },
  convertidas: { title: "Convertidas", description: "Progreso semanal de oportunidades convertidas a Cloud Sales." },
};
...
function getPreviousRow(current: PreOpp, allRows: PreOpp[], weeks: Map<string, number>) {
  ...
  return allRows.find((row) => getRowKey(row) === getRowKey(current) && (row.semanaLabel === previousWeek || row.semanaId === previousWeek)) || null;
}

function countChanges(current: PreOpp, previous?: PreOpp | null) {
  if (current.cambioEtapa && current.cambioEtapa !== "Sin cambio") return 1 + (current.variacion !== 0 ? 1 : 0);
...
function getLatestByKey(rows: PreOpp[], weeks: Map<string, number>) {
  return rows.slice().sort((a, b) => {
    const weekDiff = (weeks.get(b.semanaLabel || b.semanaId) ?? 0) - (weeks.get(a.semanaLabel || a.semanaId) ?? 0);
...
function consolidatePreOppUnits(filtered: PreOpp[], allRows: PreOpp[]): ConsolidatedUnit[] {
  ...
  return Array.from(grouped.entries()).map(([key, rows]) => {
    ...
    return {
      ...
      state,
      stage: bestRow.etapa,
      pipelineEsperado: PIPELINE_VALUE_PER_PREOPP,
      pipelineLogrado: safeNumber(latest.montoEstimado),
      latest,
      rows,
    };
...
function getAccountProductRows(units: ConsolidatedUnit[]): AccountProductRow[] {
  ...
  return Array.from(grouped.entries()).map(([key, accountUnits]) => {
    const latest = accountUnits[0].latest;
    const productStatus: AccountProductRow["productStatus"] = {};
    accountUnits.forEach((unit) => {
      productStatus[unit.productFamily] = { state: unit.state, stage: unit.stage, preopp: unit.latest };
    });
...
function getActivities(preopp: PreOpp, activities: Activity[]) {
  const fromLog = activities
    ...
    .map((activity) => ({ type: activity.tipo, detail: activity.descripcion, date: activity.fecha, origen: activity.origen }));

  if (fromLog.length) return fromLog;

  const base = [
    { type: "Actualización", detail: `Etapa real esta semana: ${preopp.etapa}`, date: preopp.ultimaActividad || "Sin fecha", origen: "Vercel_View" },
    { type: "Producto", detail: preopp.producto, date: preopp.ultimaActividad || "Sin fecha", origen: "Vercel_View" },
  ];
  if (preopp.etapaAnterior) base.push({ type: "Semana pasada", detail: `Etapa real semana pasada: ${preopp.etapaAnterior}`, date: preopp.ultimaActividad || "Sin fecha", origen: "Vercel_View" });
  if (preopp.montoEstimado > 0) base.push({ type: "Monto", detail: `Pipeline logrado: ${currency(preopp.montoEstimado)}`, date: preopp.ultimaActividad || "Sin fecha", origen: "Vercel_View" });
  if (preopp.motivoDescarte) base.push({ type: "Descarte", detail: preopp.motivoDescarte, date: preopp.ultimaActividad || "Sin fecha", origen: "Vercel_View" });
  if (preopp.senal) base.push({ type: "Señal", detail: preopp.senal, date: preopp.ultimaActividad || "Sin fecha", origen: "Vercel_View" });
  return base;
}
...
export default function DashboardClient({ sellers, preopps, activities, source, view = "overview" }: Props) {
  ...
  const weekOptions = useMemo(() => ["Todas", ...unique(preopps.map((p) => p.semanaLabel || p.semanaId))], [preopps]);
  ...
  const accountOptions = useMemo(() => ["Todas", ...unique(preopps
    ...
    .map((p) => p.cuenta))], [preopps, selectedRegion, selectedWeek, selectedSeller, selectedState, selectedProduct]);

  const sellerOptions = useMemo(() => ["Todos", ...unique(preopps
    .filter((p) => selectedRegion === "Todas" || p.region === selectedRegion)
    .filter((p) => selectedWeek === "Todas" || p.semanaLabel === selectedWeek || p.semanaId === selectedWeek)
    .filter((p) => selectedAccount === "Todas" || p.cuenta === selectedAccount)
    .filter((p) => selectedState === "Todos" || getExecutiveState(p) === selectedState)
    .filter((p) => selectedProduct === "Todos" || productFamily(p.producto) === selectedProduct || p.producto === selectedProduct)
    .map((p) => p.vendedor))], [preopps, selectedRegion, selectedWeek, selectedAccount, selectedState, selectedProduct]);

  const productOptions = useMemo(() => ["Todos", ...unique(preopps
    .filter((p) => selectedRegion === "Todas" || p.region === selectedRegion)
    .filter((p) => selectedWeek === "Todas" || p.semanaLabel === selectedWeek || p.semanaId === selectedWeek)
    .filter((p) => selectedSeller === "Todos" || p.vendedor === selectedSeller)
    .filter((p) => selectedAccount === "Todas" || p.cuenta === selectedAccount)
    .filter((p) => selectedState === "Todos" || getExecutiveState(p) === selectedState)
    .map((p) => productFamily(p.producto)))], [preopps, selectedRegion, selectedWeek, selectedSeller, selectedAccount, selectedState]);

  useEffect(() => { if (selectedSeller !== "Todos" && !sellerOptions.includes(selectedSeller)) setSelectedSeller("Todos"); }, [selectedSeller, sellerOptions]);
  useEffect(() => { if (selectedAccount !== "Todas" && !accountOptions.includes(selectedAccount)) setSelectedAccount("Todas"); }, [selectedAccount, accountOptions]);
  useEffect(() => { if (selectedProduct !== "Todos" && !productOptions.includes(selectedProduct)) setSelectedProduct("Todos"); }, [selectedProduct, productOptions]);

  const filteredPreopps = useMemo(() => preopps.filter((p) => {
  ...
  const pipelineEsperado = consolidatedUnits.length * PIPELINE_VALUE_PER_PREOPP;
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        <div className="brand"><div className="brand-mark">◎</div><div><strong>PreOpp Radar</strong><span>Monitoreo semanal de<br />pre-oportunidades</span></div></div>
        <nav className="nav" aria-label="Navegación principal">
          {navItems.map((item) => {
            const activeLink = pathname === item.href || (item.href === "/" && pathname === "/");
            return <Link key={item.id} href={item.href} className={activeLink ? "active" : ""}><span>{item.icon}</span>{item.label}</Link>;
          })}
        </nav>
        <div className="sidebar-footer"><span>Última actualización</span><strong>Semanal · automático</strong><small>Fuente: {source}</small></div>
      </aside>

      <section className="workspace">
        <header className="page-header"><div><h1>{currentTitle.title}</h1><p>{currentTitle.description}</p></div><button className="ghost-button" onClick={clearFilters}>Limpiar filtros</button></header>
        ...
        <section className="context-strip"><strong>Vista actual:</strong><span>{selectedWeek}</span><span>{selectedRegion}</span><span>{selectedAccount}</span><span>{selectedSeller}</span><span>{selectedState}</span><span>{selectedProduct}</span><span>{accountProductRows.length} cuentas</span><span>{consolidatedUnits.length} PreOpps consolidadas</span></section>
...
function Filter({ label, value, options, onChange, wide = false }: { label: string; value: string; options: string[]; onChange: (value: string) => void; wide?: boolean }) {
  return <label className={`filter-chip ${wide ? "wide-filter" : ""}`}><span>{label}</span><select value={value} onChange={(e) => onChange(e.target.value)}>{options.map((option) => <option key={option}>{option}</option>)}</select></label>;
}

function Overview({ accounts, productColumns, openDetail }: { accounts: AccountProductRow[]; productColumns: string[]; openDetail: (preopp: PreOpp) => void }) {
  return (
    <section className="panel overview-status-panel">
      <div className="panel-head"><div><h2>Estado de cuentas por producto</h2><p>Una cuenta por fila. Los iconos reflejan el estatus consolidado de cada PreOpp por producto.</p></div><Pill tone="blue">{accounts.length} cuentas visibles</Pill></div>
        ...
        <table className="status-table">
          ...
          <tbody>
            {accounts.map((account) => (
              <tr key={account.key}>
                <td data-label="Cuenta"><b>{account.cuenta}</b><small>{account.region} · {account.pais} · {account.industria}</small></td>
                <td data-label="Vendedor"><b>{account.vendedor}</b><small>{compactCurrency(account.pipelineLogrado)} logrado · {compactCurrency(account.pipelineEsperado)} esperado</small></td>
                {productColumns.map((product) => {
                  const status = account.productStatus[product];
                  return (
                    <td data-label={product} key={`${account.key}-${product}`}>
                      {status ? (
                        <button className={`status-icon status-${getStateTone(status.state)}`} title={`${stateLabel(status.state)} · ${status.stage}`} onClick={() => openDetail(status.preopp)}>
      ...
      <div className="legend status-legend">
        <span>🔎 Propuesta</span><span>🏃‍➡️ Activa</span><span>🏆 Convertida</span><span>🏃 Descartada</span><span>— Sin PreOpp</span>
      </div>
    </section>
  );
}

function PreOpps({ preopps, allRows, weeks, openDetail, mode }: { preopps: PreOpp[]; allRows: PreOpp[]; weeks: Map<string, number>; openDetail: (preopp: PreOpp) => void; mode: View }) {
  const baseRows = preopps.filter((p) => mode === "convertidas" ? ["Convertidas", "Convertida Congelada"].includes(getExecutiveState(p)) : !["Convertidas", "Convertida Congelada"].includes(getExecutiveState(p)));
  ...
  return (
    <section className="panel">
      <div className="panel-head"><div><h2>{mode === "convertidas" ? "Convertidas" : "Pre-oportunidades"}</h2><p>Comparación contra la semana anterior usando snapshot histórico. Las actividades se revisan desde “Ver detalles”.</p></div><Pill tone="blue">{rows.length} registros</Pill></div>
      <div className="table-wrap">
        <table className="data-table weekly-comparison-table">
          ...
          <tbody>{rows.map(({ current, previous, changes }) => {
            ...
            const delta = safeNumber(current.montoEstimado) - safeNumber(previousAmount);
            return (
              <tr key={`${current.id}-${current.semanaId}`} className="clickable-row" onClick={() => openDetail(current)}>
                ...
                <td data-label="Esta semana · monto">{currency(current.montoEstimado)}</td>
                ...
                <td data-label="Detalle"><button className="row-action" onClick={(event) => { event.stopPropagation(); openDetail(current); }}>Ver detalles <small>{changes} cambios</small></button></td>
              </tr>
            );
          })}</tbody>
        </table>
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Detalle de la pre-oportunidad">
      <article className="modal-card side-panel-card">
        <div className="modal-head"><div><h2>Detalles de la Pre-oportunidad</h2><p>{preopp.cuenta} · {productFamily(preopp.producto)}</p></div><button onClick={onClose} aria-label="Cerrar">×</button></div>
        <div className="detail-grid">
          ...
          <Detail label="Pipeline logrado" value={currency(preopp.montoEstimado)} />
